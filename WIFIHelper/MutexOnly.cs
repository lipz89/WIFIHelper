...
*/
        ...
        private static void SendCopyData(IntPtr hWnd, string fileName)
        {
            byte[] lpData = Encoding.UTF8.GetBytes(fileName);
            var cds = new CopyDataStruct
            {
                dwData = (IntPtr) WM_OPENNEWFILE,
                cbData = lpData.Length,
                lpData = Marshal.AllocHGlobal(lpData.Length)
            };
            ...
            try
            {
                SendMessage(hWnd, WM_COPYDATA, WM_OPENNEWFILE, ref cds);
            }
            finally
            {
        ...
        public static void ShowMutexOnly(RunWithArgsHandler runMethod, string[] args)
        {
            bool b;
            var m = new Mutex(true, ApplicationName, out b);
            if (b)
    ...
    delegate void RunWithArgsHandler(string[] args);
    delegate void RunHandler();
